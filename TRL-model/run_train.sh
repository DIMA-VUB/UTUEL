...
# ── Paths ─────────────────────────────────────────────────────────────────────
OUTPUT_DIR="TRL-model/checkpoints/${SLUG}"
LOG_DIR="TRL-model/logs"
...
(
    ...
    if $SWEEP; then
        python TRL-model/train.py --config-name sweep_optuna --multirun "${OVERRIDES[@]}" && _ec=0 || _ec=$?
    else
        python TRL-model/train.py "${OVERRIDES[@]}" && _ec=0 || _ec=$?
    fi
